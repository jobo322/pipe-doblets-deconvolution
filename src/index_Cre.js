...
(async () => {
  const pathToWrite = '/home/centos/CreQuant';
  // const pathToWrite = '/home/centos/Heid';
  // const pathToWrite = '/home/centos/Covax';// for covvax
  ...
  for (const path of paths) {
    ...
    for (let i = 0; i < experiments.length; i++) {
      ...
      const name = getName(data); // for ANPC run
...
function process(options) {
  ...
  const fromTo = { from: 4.02, to: 4.08 }; // Changed to the Creatinine region
  // const fromTo = { from: 3.0, to: 3.07 }; // Changed to the Creatinine region
  ...
  const medianOfROI = xMedian(xyExtract(xyData, {
    zones: [{ from: 11.5, to: 12.5 }], // eretic range??
  ...
  writeFileSync(join(pathToWrite, `${name}_Cre.json`), JSON.stringify([{ // for ordinary 4.06 ppm
  // writeFileSync(join(pathToWrite, `${name}_Cre_3ppm.json`), JSON.stringify([{  // for cre at 3.04 ppm
