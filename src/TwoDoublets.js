...
function process(options) {
  ...
  const fromTo = { from: 6.27, to: 6.31 };
  ...
  const medianOfROI = xMedian(xyExtract(xyData, {
    zones: [{ from: 6.25, to: 6.315 }], // bigger reference range that covers all compounds
  }).y);
  
  if (medianOfAll * 1.5 > medianOfROI) {
    console.log("medianOfAll" ,medianOfAll , "medianOfROI",medianOfROI)
    console.log("failed on median values")
    return
  };
  ...
  console.log(ranges[rangeIndex].signals[signalIndex].delta);

  if (ranges[rangeIndex].signals[signalIndex].delta.toString() ==="NaN") {
    console.log("Not Integ")
    return
  };
