...
(async () => {
  const pathToWrite = '/home/centos/UreaQuant';
  // const pathToWrite = '/home/centos/Covax/Urea';// for covvax
...
function process(options) {
  ...
  const fromTo = { from: 5.6, to: 6.0 }; // Changed to the UREA region
  ...
  const medianOfROI = xMedian(xyExtract(xyData, {
    zones: [{ from: 5.4, to: 6.5 }], // select the region that covers the xyData and the noise around it
  ...
  writeFileSync(join(pathToWrite, `${name}_Urea.json`), JSON.stringify([{ // for ordinary 4.06 ppm
